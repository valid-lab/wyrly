...
class ContainerImpl implements Container {
  ...
  #finalizeBootstrap(skipScopedPlanBuild = false): void {
    if (!skipScopedPlanBuild) {
      compileScopedDepSlotIndices(this.#resolvePlan);
    }
    this.#frozenSingletonPlan = undefined;
    if (!skipScopedPlanBuild) {
      this.#frozenScopedPlan = undefined;
  ...
  // --- Resolution pipeline (hot path: keep inline; see #resolveFromScopeByKey) ---
  ...
  #createInstance<T>(
    ...
    const deps = dynamicDepResolver(depKeys, resolveDepByKey);
    if (np.providerType === "existing") {
      return this.#resolveFromScopeByKey(
        scope,
        depKeys[0]!,
        np.useExisting!,
    ...
    return createInstanceFromSlot(scope, np, deps);
  }
